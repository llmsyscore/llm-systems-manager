...
Returns `{ok, thread, messages, read_only, active_run}`. A session reads its own threads; an admin can also read a Discord user's thread and the conversations a Forecast run held (owner `tower:forecast`). Forecast conversations come back with `read_only: true` and `active_run: null` — posting a message to one, renaming it or deleting it answers 404 `unknown thread`, and they never appear in `GET /api/tower/threads`.
...
`GET /api/tower/models` also carries `index`: `[{id, provider, hosts, loaded}]` for every model the gateway lists (`hosts` = the hosts whose catalog has it), which the Primary model choice uses for its labels.

### `POST /api/tower/models/get`
**Access:** [Admin]. **Body:** `{"key": "<list key>", "agent_id": "<from hosts>"}` (`agent_id` defaults to the first entry of `hosts`). Queues a `tower_get_model` job on that host. llama.cpp: downloads the GGUF through the host's own download path, adds a `repo:quant` section to its config.ini (existing sections are kept) and stores the same values as the model's active config profile `tower`, restarts llama-server and waits for it to list the model (up to 180 s), loads the model, waits for it to become resident, then runs the tool check and the eval. A stopped or failed llama.cpp download is removed from the host's cache again unless the repo already holds a finished GGUF; the job message says which. LM Studio: asks the host's LM Studio to download the Hugging Face repo (sent as its `https://huggingface.co/<repo>` link) at that quant (Unsloth `UD-` quants without the prefix, as LM Studio names them), follows the download job through the agent's `/lms/download/status/<job_id>` (progress, a failed or vanished job ends the manager job at once; an older agent without that route falls back to polling the model list, up to 3 h), loads it with a 32k context and a 2048 batch, then check and eval. Stop ends the manager job only; LM Studio keeps downloading until it is cancelled there. Either way the model gets the alias `Tower Model` unless it already has one. Returns `{ok, job}`; 404 for an unknown key or host, 409 while another eval or download runs, 503 without any host. Pinning is a separate `PUT /api/tower/model`.
...
### `GET /api/forecast`
Any session. 401 `unauthorized` with no session. Returns `{ok, enabled, mode, run, last_result, digest, tower, last_run, next_run, running, window_days, checks, findings, cleared}`. `run` is `null` while idle, `{state: "queued"}` while a Run now waits to start, or `{state: "running", stage: checks|investigating|closing|analysis, check, done, total, elapsed_s}` with the title of the check in hand and how many of the run's checks are finished; `running` is true whenever `run` is set, and `last_result` is the last run's one-line result ("12 found, 2 new"). `tower` is `null` while the tower effort was `off` for the last run, else `{tier, reason, model, digest_discarded}` — `digest_discarded` is true when Tower wrote a digest for the last run and the accuracy check refused it; `digest` is the last run's Tower digest (`null` when Tower wrote none). `checks` is one entry per detector — `{id, title, state: pending|collecting|ok|failed|off, have_days, min_days, found}`. `findings` (open) and `cleared` (the last 200 resolved, `status` `cleared` or `dismissed`) share a shape: `{id, host, subject, severity, summary, detail, since, predicted_at, rate, unit, confidence, suggested_action, graph, verified, status, first_seen, last_seen, resolved, thread_id, dismissed_by, tower_note, check, title}`. `thread_id` is set only while the Full-effort conversation behind the finding is kept (`manager.forecast.tower_history`); `tower_note` is `"discarded"` when Tower suggested a cause for the finding on its last run and the accuracy check refused it, else `null`. `graph` is `null` or `{source, name, host, start, end, threshold, fit: {slope_per_s, intercept}|null, agg: mean|max, limit}` — `agg` is how the page buckets the series, and `limit: false` marks `threshold` as a reference level (a benchmark baseline) rather than a limit the projection stops at.
