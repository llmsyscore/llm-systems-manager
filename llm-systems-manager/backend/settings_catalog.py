...
CATALOG: list[dict] = [
    ...
    _e("manager.tower.tool_mode", "choice", "Tool calls", "auto = native function calling where the provider supports it, otherwise a JSON block the model writes. llama.cpp needs --jinja for native. auto also falls back to the block for a model whose tool check only passed fenced.", "tower", MANAGER, choices=["auto", "native", "prompt"], hot=True),
