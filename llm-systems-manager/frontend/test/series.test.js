...
describe('tokenRate', () => {
  const { tokenRate } = series;

  test('first sample seeds the track and reports no rate', () => {
    const r = tokenRate(null, 100, 50, 1000);
    expect(r.track).toEqual({ t: 1000, gen: 100, prompt: 50 });
    expect(r.genRate).toBeNull();
    expect(r.promptRate).toBeNull();
  });

  test('computes per-second rates from the counter delta', () => {
    const seed = tokenRate(null, 100, 50, 1000).track;
    const r = tokenRate(seed, 160, 80, 7000);
    expect(r.genRate).toBeCloseTo(10);      // 60 tokens / 6 s
    expect(r.promptRate).toBeCloseTo(5);    // 30 tokens / 6 s
  });

  test('counter reset (manager restart) reseeds without a negative rate', () => {
    const seed = tokenRate(null, 100, 50, 1000).track;
    const r = tokenRate(seed, 5, 2, 7000);
    expect(r.genRate).toBeNull();
    expect(r.promptRate).toBeNull();
    expect(r.track).toEqual({ t: 7000, gen: 5, prompt: 2 });
  });

  test('a gap beyond maxGapMs reseeds instead of smearing an average', () => {
    const seed = tokenRate(null, 100, 50, 1000).track;
    const r = tokenRate(seed, 700, 350, 1000 + 31000);
    expect(r.genRate).toBeNull();
    expect(r.track.gen).toBe(700);
  });

  test('non-numeric counters keep the previous track and report no rate', () => {
    const seed = tokenRate(null, 100, 50, 1000).track;
    const r = tokenRate(seed, undefined, null, 7000);
    expect(r.track).toEqual(seed);
    expect(r.genRate).toBeNull();
  });

  test('zero delta yields zero rates (idle server)', () => {
    const seed = tokenRate(null, 100, 50, 1000).track;
    const r = tokenRate(seed, 100, 50, 7000);
    expect(r.genRate).toBe(0);
    expect(r.promptRate).toBe(0);
  });
});
