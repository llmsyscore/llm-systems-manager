...
- **Measure it:** the **Conversation eval** row in Settings › Tower assistant runs a handful of canned questions through Tower against the primary model (plain reads, a host named by alias, a two-step read, a timer, an action proposal, and a question card when two hostnames share a prefix) and scores each: the right tool with the right arguments, the right kind of reply, a real answer. More calls per question, corrections and retries mean a weaker model. Nothing changes on any host (every action card is denied, every question card takes its first choice); results are kept per model with its quant and server, listed in the Tools tab ledger, and **Export** downloads the full JSON.
- **Get a Tower model:** the row below it lists recommended models by the VRAM they need (Qwen3 8B / 14B / 32B, Gemma 3 12B / 27B, Llama 3.1 8B). **Get** downloads one on the primary llama.cpp host through the usual download path, adds it to llama.cpp, loads it, runs the tool check and the eval, and then offers **Pin as primary**; the fallback stays as configured. A model kept for Tower takes VRAM away from what the host serves, so pick the tier that leaves room for your working models.
