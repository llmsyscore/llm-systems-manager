...
from model_meta import HF_HOST, Fetcher, MetaCache, resolve_repo, ttl_for, _cfg
...
def pick_repo(target_repo: str, rows: list) -> Optional[dict]:
    ...
    for r in rows or []:
        rid = str(r.get("id") or "")
        p = params_b(rid)
        if family_prefix(rid) != fam or _is_base(rid) or p is None or p > tp * MAX_DRAFT_RATIO:
            continue
...
def pick_file(siblings: list) -> Optional[dict]:
    """Q4_K_M when present, else the smallest Q4 / Q5 / IQ4 file; projectors and other quants ignored."""
    rows = []
    for s in siblings or []:
        name = str(s.get("rfilename") or "")
        if name.lower().startswith("mmproj") or not _QUANT_RE.search(name):
            continue
        rows.append({"file": name, "size_bytes": int(s.get("size") or 0)})
    if not rows:
        return None
...
def _search(term: str, fetch) -> tuple[Optional[list], bool]:
    url = f"{HF_HOST}/api/models?search={term}&filter=gguf&sort=downloads&direction=-1&limit={SEARCH_LIMIT}"
...
def build_candidate(target_repo: str, fetch: Callable[[str], tuple[int, str]]) -> dict:
    ...
    while remaining and tried < 3:
        ...
        rid = str(repo_row["id"])
        f = pick_file(_siblings(rid, fetch) or [])
        if f is None:
            continue
        ...
        return doc
    doc["reason"] = f"no smaller GGUF of the {fam} family on Hugging Face"
    return doc
...
def register_routes(app, ctx, *, db_path: str, read_ini: Callable[[], Any],
    ...
    def llm_draft_candidates():
        ...
        if doc is None:
            doc = build_candidate(repo, fetcher_factory(token).get)
            cache.put(key, doc)
