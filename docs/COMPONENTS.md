...
| lms_load_prefs.py | Per-model LM Studio load preferences (the load options an Autotune applied), merged into /api/lmstudio/load |
...
The Manager talks to agents (over HTTPS) to collect live AI state and to proxy control commands. It talks to the Alarm Engine (over HTTPS) to forward metrics, retrieve alert state, and proxy history chart data to the browser. It also reads and writes three local SQLite files: `data/manager.db` holds the small tables — `model_benchmarks` (AI model benchmark results), `report_cards` (one row per completed GPU Report Card run), `tool_runs` (one row per completed Report Card, Benchmark, or Autotune run), Tower threads, jobs and model metadata; `data/audit.db` holds `audit_log` (a log of mutating admin actions, retention-purged at `[manager.audit].retention_days` — 60 default — with a 100,000-row backstop); `data/energy.db` holds `energy_hourly` (per-agent hourly energy and token accounting). The two write-heavy tables sit apart so their churn and backup size do not land on everything else. A manager upgraded from an earlier release renames its old `data/metrics.db` to `manager.db` and moves the two tables out on its first boot; metric history itself lives in InfluxDB via the Alarm Engine, not here.
...
A separate phone-sized app shell at `/companion` (`companion.html`, `js/companion.js`, with view-model transforms in `js/lib/companion-view.js` and sparkline helpers in `js/lib/companion-spark.js`) installs to a home screen as a Progressive Web App. Its service worker (`sw.js`) caches the app shell keyed on the manager version, and a web-push subscription delivers alarm notifications even when the app is closed. Seven screens — Home, Alerts, Tower, Energy, Models, Admin, Settings — reuse the manager's existing session-gated APIs; the Tower screen (`js/companion-tower.js`, over the drawer's `js/lib/tower-view.js` state reducer and `js/lib/sseguard.js`) drives the same thread, run and insight routes as the dashboard drawer and hides while Tower is off; control actions (model swap/pin, autopilot approval, service/agent restarts) require the admin role and confirm in a bottom sheet before running. Installing it requires the manager to serve an operator-provided (publicly trusted) TLS certificate.
