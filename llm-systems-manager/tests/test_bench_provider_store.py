...
# ── /api/benchmark/recent + DELETE /api/benchmark/results (#769) ──

def _seed_bench(conn):
    rows = [("m1", "llama", "2026-08-01T00:00:00+00:00"),
            ("m2", "vllm", "2026-08-03T00:00:00+00:00"),
            ("m3", "llama", "2026-08-02T00:00:00+00:00")]
    for mid, prov, ts in rows:
        conn.execute(
            "INSERT INTO model_benchmarks (model_id, agent_id, provider,"
            " avg_gen_tps, avg_ppt_tps, bench_tool, switches, ts)"
            " VALUES (?, 'ag', ?, 40.0, 1000.0, 'llama-bench', '[]', ?)",
            (mid, prov, ts))
    conn.commit()


def test_recent_orders_newest_first_across_providers(monkeypatch):
    conn = _mem_db()
    monkeypatch.setattr(manager_mod, "get_db", lambda: conn)
    manager_mod.init_db()
    _seed_bench(conn)
    r = _client().get("/api/benchmark/recent")
    assert r.status_code == 200
    out = r.get_json()["results"]
    assert [x["model_id"] for x in out] == ["m2", "m3", "m1"]
    assert out[0]["provider"] == "vllm" and out[0]["agent_id"] == "ag"


def test_recent_limit_clamped(monkeypatch):
    conn = _mem_db()
    monkeypatch.setattr(manager_mod, "get_db", lambda: conn)
    manager_mod.init_db()
    _seed_bench(conn)
    c = _client()
    assert len(c.get("/api/benchmark/recent?limit=1").get_json()["results"]) == 1
    assert len(c.get("/api/benchmark/recent?limit=abc").get_json()["results"]) == 3


def test_results_clear_deletes_all_rows(monkeypatch):
    conn = _mem_db()
    monkeypatch.setattr(manager_mod, "get_db", lambda: conn)
    manager_mod.init_db()
    _seed_bench(conn)
    c = _client()
    r = c.delete("/api/benchmark/results")
    assert r.status_code == 200 and r.get_json()["ok"] is True
    assert c.get("/api/benchmark/recent").get_json()["results"] == []
